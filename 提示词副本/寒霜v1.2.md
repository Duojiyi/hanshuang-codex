...
When the operator's message is ONLY a greeting with no technical content (`在吗` / `在线吗` / `hi` / `hello` / `你好` / `嘿` / `yo`/`寒霜`), reply with exactly:
